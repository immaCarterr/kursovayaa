...
int CheckExistLog(string path, string log, string pass) {
	int flag = 1;
	fstream fs;
	fs.open(path);
	if (fs.is_open()) {
...
void reg(string path) {
	User user;
	bool flag = true;
	while (flag) {
		system("cls");
		cout << "Введите логин: "; cin >> user.login;
		int ch = CheckExistLog(path, user.login, "");
		if (ch == 1 || ch == -1) {
...
int authorization(string path) {
	...
	while (flag) {
		...
		cout << "Введите логин: "; cin >> user.login;
		cout << "\nВведите пароль: "; Password(user.password);
		ch = CheckExistLog(path, user.login, user.password);
		if (ch == 2) {
			res = 1;
...
void DelUsers() {
	...
	if (fs.is_open()) {
		...
		while (true) {
			...
			if (CheckExistLog("Users.txt", log, "") != 0) {
...
void ShowBuyers() {
	...
	if (fs.is_open()) {
		...
		cout << "|" << setw(31) << setfill(' ') << "|" << setw(24) << setfill(' ') << "|" << setw(33) << setfill(' ') << "|" << endl;

		cout << resetiosflags(ios::adjustfield);
		cout.setf(ios::left);
		cout << "|" << setw(30) << setfill(' ') << " ФИО" << "|" << setw(23) << setfill(' ') << " Номер" << "|" << setw(32) << setfill(' ') << " Адрес" << "|" << endl;
		...
		for (int i = 0; i < D.size(); i++)
		{
			...
			cout << "| " << setw(29) << setfill(' ') << fio << "| " << setw(22) << setfill(' ') << D[i].GetNumber() << "| " << setw(31) << setfill(' ') << adr << "|" << endl;
